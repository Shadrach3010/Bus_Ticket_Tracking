...
async function loadState(supabase) {
  const [routesQ,busesQ,tripsQ,ticketsQ,paymentsQ,manifestQ,reportsQ,feedbackQ,logsQ,usersQ] = await Promise.all([
    ...
    supabase.from("buses").select("*,routes(*),profiles(*)").order("created_at"),
    supabase.from("trips").select("*,routes(*),buses(*),profiles(*)").order("departure_at"),
    supabase.from("tickets").select("*,routes(*),buses(*),passenger:profiles!tickets_passenger_id_fkey(*),validator:profiles!tickets_validated_by_fkey(*)").order("purchased_at", { ascending: false }),
    supabase.from("payments").select("*,tickets(*,routes(*)),profiles(*)").order("paid_at", { ascending: false }),
    supabase.from("manifest_passengers").select("*,tickets(*,routes(*),profiles(*)),trips(*)").order("created_at", { ascending: false }),
    supabase.from("incident_reports").select("*,profiles(*),trips(*)").order("submitted_at", { ascending: false }),
    supabase.from("feedback").select("*,profiles(*),routes(*)").order("created_at", { ascending: false }),
    supabase.from("validation_logs").select("*,tickets(*,routes(*),profiles(*)),profiles(*),buses(*)").order("scanned_at", { ascending: false }),
  ...
  const tickets = (ticketsQ.data ?? []).map((t) => ({
    ...
    travelDate:dateLabel(t.travel_date), fare:Number(t.fare), seatNumber:t.seat_number,
  ...
  return {
    ...
    buses:(busesQ.data ?? []).map((b) => ({ id:b.id,plate:b.plate,model:b.model,capacity:b.capacity,routeId:b.route_id ?? "",routeName:routeLabel(b.routes),conductorId:b.conductor_id ?? undefined,conductorName:b.profiles ? fullName(b.profiles) : undefined,status:b.status,amenities:b.amenities })),
    tickets,
    payments:(paymentsQ.data ?? []).map((p) => ({ id:p.id,ticketReference:p.tickets?.reference ?? "",passengerName:fullName(p.profiles),passengerEmail:p.profiles?.email,route:routeLabel(p.tickets?.routes),amount:Number(p.amount),method:p.method,status:p.status,date:iso(p.paid_at),transactionRef:p.transaction_ref })),
    ...
    incidentReports:(reportsQ.data ?? []).map((r) => ({ id:r.id,tripId:r.trip_id,conductorName:fullName(r.profiles),type:r.type,severity:r.severity,title:r.title,description:r.description,status:r.status,submittedAt:iso(r.submitted_at) })),
    feedbackList:(feedbackQ.data ?? []).map((f) => ({ id:f.id,passengerName:fullName(f.profiles),passengerEmail:f.profiles?.email ?? "",route:routeLabel(f.routes),rating:f.rating,category:f.category,comment:f.comment,date:dateLabel(f.created_at),status:f.status })),
    validationLogs:(logsQ.data ?? []).map((l) => ({ id:l.id,ticketReference:l.ticket_reference,passengerName:l.tickets?.passenger_name ?? "Unknown",route:routeLabel(l.tickets?.routes),status:l.status,timestamp:iso(l.scanned_at),conductorName:fullName(l.profiles),bus:l.bus_id ?? "" })),
...
export async function GET() {
  ...
  try { return NextResponse.json({ state:data?.claims?.sub ? await loadState(supabase) : await loadPublicState(supabase) }); }
