...
export default function ConductorProfilePage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("conductor_profile_data");
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        setProfileImage(parsed.profileImage || "");
        setFirstName(parsed.firstName || "Mohamed");
        setLastName(parsed.lastName || "Bangura");
        setPhone(parsed.phone || "+23276222333");
        setBadgeNumber(parsed.badgeNumber || "CND-SL-4412");
        setLicenseNumber(parsed.licenseNumber || "LIC-TRANS-88901");
        setAssignedBus(parsed.assignedBus || "BUS-18");
      } catch {
        // fallback
      }
    }
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    localStorage.setItem("conductor_profile_data", JSON.stringify(payload));
